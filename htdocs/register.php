...
function sendOTP($data) {
    ...
    if ($error) {
        return ['status' => 'error', 'message' => 'خطأ في الاتصال: ' . $error];
    }
    
    if ($httpCode != 200) {
        return ['status' => 'error', 'message' => 'فشل الاتصال بالخادم (HTTP ' . $httpCode . ')'];
    }
                ...
                <form method="POST" id="register-form">
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                            <label class="relative cursor-pointer">
                                ...
                                <div class="border-2 border-gray-200 peer-checked:border-blue-500 peer-checked:bg-blue-50 rounded-xl p-5 text-center transition-all">
                        ...
                        <label class="relative cursor-pointer block">
                            ...
                            <div class="border-2 border-dashed border-gray-200 peer-checked:border-blue-500 peer-checked:bg-blue-50 rounded-xl p-3 text-center transition-all">
                    ...
                    <div id="entity-fields" class="hidden mb-8">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <label class="relative cursor-pointer">
                                ...
                                <div class="border-2 border-gray-200 peer-checked:border-blue-500 peer-checked:bg-blue-50 rounded-xl p-4 text-center transition-all">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="text" name="name" required value="<?= clean($_POST['name'] ?? '') ?>" class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500" placeholder="أدخل اسمك الكامل">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">البريد الإلكتروني <span class="text-red-500">*</span></label>
                            <input type="email" name="email" required value="<?= clean($_POST['email'] ?? '') ?>" class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500" placeholder="your@email.com">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <div class="relative">
                                <input type="password" name="password" id="reg-password" required class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500" placeholder="8 أحرف على الأقل">
                                <button type="button" onclick="togglePassword('reg-password')" class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="password" name="confirm_password" id="reg-confirm-password" required class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500" placeholder="أعد كتابة كلمة المرور">
                                <button type="button" onclick="togglePassword('reg-confirm-password')" class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">
                                    <i class="fas fa-eye"></i>
                                </button>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="tel" name="phone" value="<?= clean($_POST['phone'] ?? '') ?>" class="w-full border border-gray-300 rounded-xl px-4 py-3" placeholder="+966 5XX XXX XXXX">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">واتساب</label>
                            <input type="tel" name="whatsapp" value="<?= clean($_POST['whatsapp'] ?? '') ?>" class="w-full border border-gray-300 rounded-xl px-4 py-3" placeholder="+966 5XX XXX XXXX">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <select name="country_id" id="reg-country" required onchange="loadCities('reg-country', 'reg-city')" class="w-full border border-gray-300 rounded-xl px-4 py-3 bg-white">
                                <option value="">اختر الدولة</option>
                                <?php foreach ($countries as $country): ?>
                                <option value="<?= $country['id'] ?>" <?= (($_POST['country_id'] ?? '') == $country['id']) ? 'selected' : '' ?>>
                                    <?= $country['flag'] ?> <?= htmlspecialchars($country['name_ar']) ?>
                                </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="city_id" id="reg-city" required class="w-full border border-gray-300 rounded-xl px-4 py-3 bg-white">
                    ...
                    <div id="shop-fields" class="hidden">
                        <div class="border-t border-gray-100 pt-4 mt-4">
                            <h3 class="font-bold text-gray-700 mb-3"><i class="fas fa-store text-blue-500 ml-2"></i>معلومات المحل</h3>
                            <div class="grid grid-cols-1 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">اسم المحل <span class="text-red-500">*</span></label>
                                    <input type="text" name="shop_name" value="<?= clean($_POST['shop_name'] ?? '') ?>" class="w-full border border-gray-300 rounded-xl px-4 py-3" placeholder="مثال: متجر البناء الفاخر">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">عنوان المحل</label>
                                    <input type="text" name="shop_address" value="<?= clean($_POST['shop_address'] ?? '') ?>" class="w-full border border-gray-300 rounded-xl px-4 py-3" placeholder="الشارع والحي...">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="contractor-fields" class="hidden">
                        <div class="border-t border-gray-100 pt-4 mt-4">
                            <h3 class="font-bold text-gray-700 mb-3"><i class="fas fa-hard-hat text-blue-500 ml-2"></i>التخصصات</h3>
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
                                <?php foreach (getProjectCategories() as $cat => $icon): ?>
                                <label class="flex items-center gap-2 bg-gray-50 hover:bg-blue-50 border border-gray-200 rounded-lg px-3 py-2 cursor-pointer transition">
                                    <input type="checkbox" name="specializations[]" value="<?= htmlspecialchars($cat) ?>" class="text-blue-600">
                                    <span class="text-sm"><?= $icon ?> <?= htmlspecialchars($cat) ?></span>
                                </label>
                                <?php endforeach; ?>
                            </div>
                        </div>
                    ...
                    <div class="text-center mt-4 text-sm text-gray-500">
                        لديك حساب بالفعل؟ <a href="<?= SITE_URL ?>/login.php" class="text-blue-600 font-medium">تسجيل الدخول</a>
                    </div>
                </form>
            </div>
        </div>
